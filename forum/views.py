...
def comment(request,query_id):
	if request.POST:
		...
		if form1.is_valid():
			name=form.cleaned_data['name']
			query=Query.objects.get(id=query_id)
			done=Notification.objects.create(name=name,query=query)
...
def notifications(request):
	...
	return render(request, '/home/nikhil/Desktop/CCBT/ccbt/forum/templates/notifications.html',args)

def latest(request):
	latest_query_list = Query.objects.order_by('-pub_date')[:5]
	context = {'latest_query_list': latest_query_list,}
	return render(request,'/home/nikhil/Desktop/CCBT/ccbt/forum/templates/latest.html',context)
